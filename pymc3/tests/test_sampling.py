...
class TestSample(SeededTest):
    ...
    def test_parallel_start(self):
        with self.model:
            tr = pm.sample(5, tune=0, njobs=2, start=[{'x': [10, 10]}, {'x': [-10, -10]}],
                           random_seed=self.random_seed)
        assert tr.get_values('x', chains=0)[0][0] > 0
        assert tr.get_values('x', chains=1)[0][0] < 0

    def test_sample_discard_draws(self):
        with self.model:
            trace = pm.sample(draws=100, tune=50, njobs=1)
            assert len(trace) == 50
            trace = pm.sample(draws=100, tune=50, njobs=1, discard_tuned_samples=False)
